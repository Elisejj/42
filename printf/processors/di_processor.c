...
/*   Updated: 2021/01/18 20:32:06 by lmeribal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int put_if_neg_width(p_struct specification, int num, int space_len, int zero_len, size_t *print_length)
{
	int number_len;
...
static int put_if_flag(p_struct specification, int num, int space_len, int zero_len)
{
...
int di_processor(p_struct specification, int num, size_t *print_length)
{
	...
	if (!neg_width)
	{
		*print_length += zero_len;
		*print_length += space_len;
		*print_length += get_num_len(specification, num);
	}
	...
	if (neg_width == 1)
		put_if_neg_width(specification, num, space_len, zero_len, &(*print_length));
	else
